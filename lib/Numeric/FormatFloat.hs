...
-- No implicit Prelude: without this the module picks one up, and Prelude ->
-- Data.Double -> Data.FloatW -> here is a cycle the moment Show FloatW is
-- defined in terms of showFloat (which is what it should be defined as).
import Prelude()
import Primitives
...
import Data.String
import Data.Tuple
import Text.Show(ShowS, showChar, show, showString)
import Numeric.Show

showFloat :: (RealFloat a, RealFrac a) => a -> ShowS
showFloat x = showString (formatRealFloat FFGeneric Nothing x)

-- These are the format types.  This type is not exported.
data FFFormat = FFExponent | FFFixed | FFGeneric

formatRealFloat :: (RealFloat a, RealFrac a) => FFFormat -> Maybe Int -> a -> String
formatRealFloat fmt decs x = formatRealFloatAlt fmt decs False x

formatRealFloatAlt :: (RealFloat a, RealFrac a) => FFFormat -> Maybe Int -> Bool -> a
                 -> String
...
floatToDigits :: (RealFloat a, RealFrac a) => Integer -> a -> ([Int], Int)
   ...
   let
    ...
    k0 =
     if b == 2 && base == 10 then
        ...
        -- The log is taken in the caller's own float type, not a hardwired
        -- Float: naming Float here made this module import Data.Float, and
        -- Data.Float is a newtype over FloatW, so Show FloatW could never be
        -- written in terms of showFloat without a module cycle.  RealFloat
        -- already implies Floating, so the type variable does the job.
        ceiling ((log (fromInteger (f+1) `asTypeOf` x) +
...
showEFloat :: (RealFloat a, RealFrac a) => Maybe Int -> a -> ShowS
showFFloat :: (RealFloat a, RealFrac a) => Maybe Int -> a -> ShowS
showGFloat :: (RealFloat a, RealFrac a) => Maybe Int -> a -> ShowS
showFFloatAlt :: (RealFloat a, RealFrac a) => Maybe Int -> a -> ShowS
showGFloatAlt :: (RealFloat a, RealFrac a) => Maybe Int -> a -> ShowS
...
showHFloat :: (RealFloat a, RealFrac a) => a -> ShowS
