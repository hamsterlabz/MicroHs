...
-- {-# LAZY f g .. #-}: the lexer hands us a TPragma whose string is "LAZY f g .."
pLazyPragma :: P EDef
pLazyPragma = do
  t <- satisfy "{-# LAZY #-}" isLazy
  pure $ LazyP [ mkIdent w | w <- drop 1 (words (pragStr t)) ]
  where isLazy (TPragma _ s) = takeWhile (/= ' ') s == "LAZY"
        isLazy _ = False
        pragStr (TPragma _ s) = s
        pragStr _ = ""
