-- CountdownMorphM.hs - Countdown in the exp3_8m idiom.
--
-- `subs` already names its recursive result once (`let yss = subs xs`), which
-- is the idiom applied by hand.  `solns` does not: it binds ems and then calls
-- results ns a second time.
module CountdownMorphM where
...
-- `results ns` was written twice: bound to ems and then called again.  Two
-- occurrences are two graph nodes, so the whole search is run twice.
