-- ClausifyHylo.hs - Clausify restructured: formula unfolded straight into clauses.
module ClausifyHyloProf where
...
-- The distributed CNF tree is built by disin/din/din2 and immediately taken
-- apart again by split/clauses -- a build-then-consume, and the built thing is
-- exponentially larger than the input.  Unfolding the formula DIRECTLY into
-- clauses skips it: distribution becomes the cross product of the two clause
-- lists, so disin, din, din2, split and clauses all disappear.
--
--   cls (Con p q) = cls p ++ cls q
--   cls (Dis p q) = [ mrg c1 c2 | c1 <- cls p, c2 <- cls q ]
--
-- mrg keeps the two symbol lists sorted, exactly as `clause` did with insert.
mrg (c1, a1) (c2, a2) = (foldr insert c2 c1, foldr insert a2 a1)

cls (Con p q)       = cls p ++ cls q
cls (Dis p q)       = concatMap (\ c1 -> map (mrg c1) (cls q)) (cls p)
cls (Sym s)         = [([s], [])]
cls (Neg (Sym s))   = [([], [s])]
cls _               = [([], [])]
...
main = --let p = eqv (eqv a $ eqv a a)
       ...
       display $ clausify $ foldr Con (Sym (0::Int)) $ replicate 2000 (eqv (eqv (Sym (0::Int)) $ eqv (Sym (0::Int)) (Sym (0::Int))) (eqv (Sym (0::Int)) $ eqv (Sym (0::Int)) (Sym (0::Int))))
