...
pragma loc cs =
  let as = map toUpper $ takeWhile isAlpha $ dropWhile isSpace cs
      ...
      -- original-case body up to the closing #-}, e.g. "LAZY foldr sum"
      body = unwords (words (takeWhile (/= '#') cs))
  in  case as of
        "SOURCE" -> TPragma loc as : skip
        -- loc here points at the '#'; back up 2 to the '{' column so layout treats a
        -- standalone {-# LAZY #-} declaration as starting at the decl indent (inserts ';').
        "LAZY"   -> TPragma (addCol loc (-2)) body : skip   -- carries the fn names for the parser
        _ -> skip
