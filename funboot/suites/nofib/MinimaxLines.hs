-- MinimaxFused.hs - the four tree passes fused into one walk.
...
-- static walked the whole 3x3 board once per win line -- eight lines, nine
-- cells, seventy-two cell visits per node -- to compute eight sums.  But the
-- eight `wins` ARE the three rows, three columns and two diagonals, all
-- unweighted, so the nine cells determine every line.  Read the board once,
-- score each cell once, and every line is an addition.  interpret still
-- short-circuits on the first decisive line, and the list is lazy, so the later
-- sums are only forced if reached.
              ...
              case r3 of
                (g : h : i : _) ->
                  let sa = scorePiece a 1
                      ...
                      si = scorePiece i 1
                  in  interpret 0
                        [ eval (sa + sb + sc)      -- win1..3: rows
                        , eval (sd + se + sf)
                        , eval (sg + sh + si)
                        , eval (sa + sd + sg)      -- win4..6: columns
                        , eval (sb + se + sh)
                        , eval (sc + sf + si)
                        , eval (sa + se + si)      -- win7: diagonal
                        , eval (sc + se + sg) ]    -- win8: anti-diagonal
                _ -> interpret 0 (map (score board) wins)
            _ -> interpret 0 (map (score board) wins)
        _ -> interpret 0 (map (score board) wins)
    _ -> interpret 0 (map (score board) wins)
...
-- score is the innermost loop: it runs for each of the eight win lines at every
-- node.  As written it builds a 3x3 list of Ints (map2 (map2 scorePiece)), then
-- a 3-element list (map sum), then folds that away -- three intermediate
-- structures per line, twenty-four per node, all of them consumed immediately.
-- One fold over the two shapes together builds none of them.
score :: Board -> Win -> Evaluation
score board win = eval (goRows 0 board win)
  where
    goRows acc []       _        = acc
    goRows acc _        []       = acc
    goRows acc (r : rs) (w : ws) = goRows (goCells acc r w) rs ws

    goCells acc []       _        = acc
    goCells acc _        []       = acc
    goCells acc (p : ps) (c : cs) = goCells (acc + scorePiece p c) ps cs
...
alternate player f g board =
  ...
  else if eqEval st XWin then []
  else if eqEval st OWin then []
  else move : alternate opposition g f board'
        where
        st = static board          -- was recomputed for each test
...
-- bestMove was  mise f g . cropTree . mapTree static . searchTree p
-- i.e. an UNFOLD (repTree, cut by prune) followed by THREE walks of the same
-- tree: mapTree scores every node, cropTree paramorphically decides -- from a
-- node's own value -- whether to keep its children at all, and mise folds the
-- whole thing away with the two players alternating.
--
-- Fused into one walk.  The tree is never built; the crop test is an inline
-- case on the node's score; the player swap (opposite p) and the algebra swap
-- (g f) ride down together, which is what repTree and mise were each doing
-- separately.
bestMove :: Piece -> Player -> Player -> Board -> Evaluation
bestMove p0 f0 g0 b0 = go simDepth p0 f0 g0 b0
  where
    go d p f g b =
      let a = static b
      in  if d == 0
            then a                                  -- prune
                   ...
                   Score _ -> case newPositions p b of
                                []   -> a
                                kids -> foldr f (g OWin XWin)
                                          (map (go (d - 1) (opposite p) g f) kids)
                   _       -> a                     -- cropTree: decisive, stop
