...
GHCPKGS= -package mtl -package pretty -package haskeline -package process -package time -package ghc-prim -package containers -package deepseq -package directory -package text -package filepath
...
# The bare-metal collector is BUNDLED into the emitted .S (--bare), not a
# separate object the build scripts link.  Source of truth stays the C file;
# this rule regenerates the embedded-assembly module from it.  .file and
# .attribute lines are stripped so the outer listing's -march governs.
# extra -D for the bundled collector, e.g. GCDEF=-DGC_SURVEY.
# MUST BE EMPTY for every gated image: the survey costs cycles and prints.
GCDEF?=
GCC32= /opt/riscv/bin/riscv32-unknown-elf-gcc
src/MicroHs/FunGc.hs:	funboot/rv32/fun_gc_cheney.c tools/gen_fungc.py
	$(GCC32) -march=rv32ima_zicsr_zifencei_zicbom -mabi=ilp32 -O2 \
	  $(GCDEF) -ffreestanding -nostdlib -S -o - $< | python3 tools/gen_fungc.py > $@

# Compile mhs with ghc
bin/gmhs:	src/*/*.hs ghc/*.hs ghc/*/*.hs ghc/*/*/*.hs src/MicroHs/FunGc.hs
	@mkdir -p bin
