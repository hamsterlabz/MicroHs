...
import Data.Int
import Data.List_Type          -- fun backend: list recursion for String Eq/Ord
...
-- fun backend: no sequal/scmp primitive (they walk the cons list); do it in
-- Haskell via Char ==/compare.
instance Eq String where
  (==) = eqString

eqString :: String -> String -> Bool
eqString [] [] = True
eqString (x:xs) (y:ys) = primCharEQ x y && eqString xs ys
eqString _ _ = False

instance Ord String where
  compare =  cmpString
  x <  y  =  case cmpString x y of { LT -> True; _ -> False }
  x <= y  =  case cmpString x y of { GT -> False; _ -> True }
  x >  y  =  case cmpString x y of { GT -> True; _ -> False }
  x >= y  =  case cmpString x y of { LT -> False; _ -> True }

cmpString :: String -> String -> Ordering
cmpString [] [] = EQ
cmpString [] _  = LT
cmpString _  [] = GT
cmpString (x:xs) (y:ys) = case primCharCompare x y of { EQ -> cmpString xs ys; o -> o }
