...
import Prelude hiding(lookup, mapM, null)
...
singleton :: forall a . Int -> a -> Map a
singleton i a = One i a

elems :: forall v . Map v -> [v]
elems = map snd . toList

keys :: forall v . Map v -> [Int]
keys = map fst . toList

toList :: forall v . Map v -> [(Int, v)]
toList t = to t []
  where
    to Nil q = q
    to (One k v) q = (k, v):q
...
fromList :: forall v . [(Int, v)] -> Map v
fromList = fromListWith const

fromListWith :: forall v . (v -> v -> v) -> [(Int, v)] -> Map v
...
insert :: forall a . Int -> a -> Map a -> Map a
insert = insertWith const

insertWith :: forall a . (a -> a -> a) -> Int -> a -> Map a -> Map a
...
delete :: forall a . Int -> Map a -> Map a
delete k = del
  where
    del Nil = Nil
...
extractMin :: forall a . Map a -> (Int, a, Map a)
...
extractMax :: forall a . Map a -> (Int, a, Map a)
...
balance :: forall a . Map a -> Int -> a -> Map a -> Map a
...
balance left key val right
  | size right > omega * size left =
      case right of
        (Node rl _ _ _ rr) | size rl < alpha*size rr -> node left key val right
                           | otherwise -> node left key val right
        _ -> node left key val right
  | size left > omega * size right =
      case left of
        (Node ll _ _ _ lr) | size lr < alpha*size ll -> node left key val right
                           | otherwise -> node left key val right
        _ -> node left key val right
  | otherwise = node left key val right


singleL :: forall a . Map a -> Int -> a -> Map a -> Map a
singleL l k v (Node rl _ rk rv rr) = node (node l k v rl) rk rv rr
singleL _ _ _ _ = undefined

singleR :: forall a . Map a -> Int -> a -> Map a -> Map a
singleR (Node ll _ lk lv lr) k v r = node ll lk lv (node lr k v r)
singleR _ _ _ _ = undefined

doubleL :: forall a . Map a -> Int -> a -> Map a -> Map a
...
doubleR :: forall a . Map a -> Int -> a -> Map a -> Map a
