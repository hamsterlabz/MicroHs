...
import Data.Word
import Text.Show
import Numeric.FormatFloat(showFloat)
...
-- primFloatWShow is the C host's dtoa, and the fun backend has no host: it
-- shows up as _unimpprim_fshow and traps the moment a program formats a float.
-- Numeric.FormatFloat.showFloat is the Haskell report's own algorithm (what
-- this comment always said it should be), so it also agrees with GHC digit for
-- digit -- which a hand-rolled dtoa would not.
...
-- WHICH FLOAT, not which machine.  This picked the 32-bit decoders when
-- _wordSize was 32, which is right for the C runtime (FloatW is float on a
-- 32-bit host and double on a 64-bit one) and WRONG wherever the two come
-- apart: the rv64g fun backend is a 64-bit machine whose FloatW is binary32
-- (its own ops are fadd.s / fsqrt.s / fcvt.w.s).  There it chose
-- decodeFloat64, so decodeFloat read a binary32 value with the binary64
-- decoder and every RealFrac operation built on it -- properFraction, and so
-- truncate, round, floor, ceiling -- returned garbage, while raw arithmetic
-- and comparisons stayed correct because they never decode.
--
-- The float width is observable rather than assumed: 1.0 is 0x3F800000 in
-- binary32 and 0x3FF0000000000000 in binary64, read through the same raw-bits
-- primitive the decoders use.
_floatWSize :: Int
_floatWSize = if primWordFromFloatWRaw (1.0 :: FloatW) == (0x3F800000 :: Word)
              then 32 else 64

flt :: forall a . a -> a -> a
flt f d | _floatWSize == 32 = f
        | otherwise         = d
...
decodeFloat32 x =
  ...
  in  if expn == 0 then
        ...
        -- ordinary number, add hidden bit.
        -- binary32: 23 mantissa bits, so the hidden bit is bit 23
        -- (0x00800000) and the scaled mantissa costs -23; the exponent is
        -- offset-127.  This read 0x00400000 and -22, one bit low in BOTH,
        -- which is why 100.0 came back as (0x480000, -16) = 72.0: bit 22 is
        -- already part of that mantissa, so the hidden bit was never added
        -- and the exponent was one too large.  (decodeFloat64 beside it has
        -- the right shape -- 0x0010000000000000 and -1023-52 -- and this is
        -- that code with the widths not carried across.)
