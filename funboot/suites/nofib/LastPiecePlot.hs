-- LastPiecePlot.hs - a COPY of LastPiece.hs (nofib spectral/last-piece), plotted (SPJ's pentomino puzzle),
...
-- MODIFIED ONLY AT THE BOUNDARY.  The puzzle is LastPiece.hs untouched --
-- the same search, the same pieces, the same 8x8 board.  LastPiece.hs renders
-- the Solution tree to a Doc and hashes it; this walks the SAME tree the same
-- way display does (Soln draws, Choose recurses, Fail is skipped) and draws
-- each solved board as a frame.  So the frames are the solutions the
-- benchmark finds, in the order it finds them.
--
-- Two deliberate differences: 8 pieces rather than the RTL sim scale of 4
-- (upstream is 13), and the frames are capped so a long search cannot fill
-- the disk.
module LastPiecePlot where
import Prelude()
import NanoPrelude

foreign import ccall "sdl_open_cps"  sdlOpen  :: Int -> a -> a
foreign import ccall "sdl_board_cps" sdlBoard :: Int -> a -> a
foreign import ccall "sdl_cell_cps"  sdlCell  :: Int -> a -> a
foreign import ccall "sdl_frame_cps" sdlFrame :: Int -> a -> a
foreign import ccall "sdl_show_cps"  sdlShow  :: Int -> a -> a

plotWin, plotPieces, plotFrames :: Int
plotWin    = 640
plotPieces = 8
plotFrames = 120
...
solutions :: Solution
solutions = search (1,2) Female initialBoard (takeP plotPieces initialPieces)
...
-- the same walk display does, collecting the solved boards
solnBoards :: Solution -> [Board]
solnBoards (Soln bd)    = [bd]
solnBoards (Choose ss)  = foldr (\s acc -> append (solnBoards s) acc) [] ss
solnBoards (Fail _ _)   = []

-- one cell per square, row-major from the top, exactly the order displayBoard
-- prints; an empty square is 0 (black), a piece takes its id as a colour
drawRow :: Board -> Int -> [Int] -> a -> a
drawRow _  _ []       k = k
drawRow bd n (c:cs)   k =
  case check bd (n,c) of
    Just pid -> sdlCell ((pid - 64) * 19) (drawRow bd n cs k)
    Nothing  -> sdlCell 0                 (drawRow bd n cs k)

drawBoard :: Board -> [Int] -> a -> a
drawBoard _  []     k = k
drawBoard bd (r:rs) k = drawRow bd r (fromTo 1 maxCol) (drawBoard bd rs k)

plotBoards :: [Board] -> a -> a
plotBoards []       k = k
plotBoards (bd:bds) k =
  drawBoard bd (fromToDown maxRow 1) (sdlFrame 0 (plotBoards bds k))

main :: Int
main = sdlOpen ((plotWin * 65536) + plotWin)
         (sdlBoard maxCol
           (plotBoards (takeP plotFrames (solnBoards solutions)) (sdlShow 0 0)))
