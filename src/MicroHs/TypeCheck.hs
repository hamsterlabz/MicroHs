...
-- Nano-vs-full Prelude probe: NanoPrelude modules import no typeclass
-- machinery, so the builtin fromInteger is simply not in scope there.
-- Numeric-literal elaboration keys off this (see tcLit LInteger).
hasFromInteger :: T Bool
hasFromInteger = do
  env <- gets valueTable
  case stLookup "" (mkBuiltin noSLoc "fromInteger") env of
    Right _ -> return True
    Left _  -> return False
...
addValueClass ctx iCls vks fds ms = do
  ...
  let
      meths = [ b | b@(Sign _ _) <- ms ]
      -- one constructor FIELD per method NAME: a grouped signature
      -- (zeroS, one :: a) declares several methods sharing one type, and
      -- taking one field per Sign left the dictionary short -- the
      -- instance side (expandInst) builds one argument per method id, so
      -- the constructor was over-applied and the leftover default-method
      -- argument surfaced as an unresolvable class constraint.
      methTys = concatMap (\ (Sign is t) -> map (const t) is) meths
      methIds = concatMap (\ (Sign is _) -> is) meths
          ...
          case lit of
            LInteger i -> do
              ...
              case mex of
                ...
                -- NanoPrelude is THE standard fun prelude: there is no Num/Integer
                -- class there, so in a nano module every remaining literal defaults
                -- to Int (raw LInt), never fromInteger. In a FULL-Prelude module
                -- (fromInteger in scope: typeclass machinery present) the literal
                -- takes the standard fromInteger path - application arguments are
                -- INFERRED, so forcing LInt here manufactured a deferred (Int ~ a)
                -- equality that rejected every module importing Data.Num.
                _ -> do
                  full <- hasFromInteger
                  if full
                    then do
                    ...
                    else tcLit mt loc (LInt (fromInteger i))
            LRat r -> do
              mex <- getExpected mt
              case mex of
