...
-- the same bytes, viewed as Chars: a synonym, exactly as the real library
-- has it, so a value from Data.ByteString is one of these too
type ByteString = B.ByteString

-- the Char view of pack/unpack: the bytes are the same, the elements are
-- Chars rather than Word8s
...
hPutStrLn h s = hPutStr h s >> hPutStr h (coerce (B.singleton 10))
