...
desugar flags tm =
  setBindings tm (yrecs flags $ map lazier $ checkDup $ concatMap (dsDef flags (tModuleName tm)) (tBindingsOf tm))

-- --yrec: TOP-LEVEL recursion through Y.  The rule is
--     foo = \x -> ... foo ... ==> foo = Y (\self -> \x -> ... self ...)
-- with the binder named `self` in every case.
--
-- A local recursive binding already goes through Y -- letRecE below wraps it as
-- `Y (\i -> e)`.  A TOP-LEVEL recursive binding does not: dsDef emits (f, body)
-- and the body simply names its own global, so the knot is a self-reference in
-- the emitted graph and no fn.y is ever executed for it.  This rewrites
--     foo = \x -> 1 + foo (x-1)
-- into
--     foo = Y (\self -> \x -> 1 + self (x-1))
-- which is letRecE applied one level up, except that letRecE re-binds the
-- definition's OWN identifier and this binds `self`, so the occurrences are
-- renamed by substExp rather than captured by shadowing.
--
-- Direct self-reference only.  Mutual recursion between two top-level bindings
-- (f names g, g names f) has neither body naming ITSELF, so nothing here fires
-- and those keep the self-reference form; tupling them the way mutualRec does
-- for local bindings would be a separate transform.
yrecs :: Flags -> [LDef] -> [LDef]
yrecs flags ds
  | yrec flags = map yrecDef ds
  | otherwise  = ds

yrecDef :: LDef -> LDef
yrecDef ie@(i, e)
  | i `elem` freeVars e =
      (i, App (Lit (LPrim "Y")) (Lam selfIdent (substExp i (Var selfIdent) e)))
  | otherwise           = ie

-- The recursive binder is named `self`, the same name for EVERY definition --
-- not the definition's own identifier.  Reusing `i` as the binder would tie
-- the knot by shadowing and need no substitution, but it leaves each lambda
-- carrying its own name.
selfIdent :: Ident
selfIdent = mkIdent "self"
...
dsDef flags mn adef =
  case adef of
    ...
    ForImp ie i t -> [(i, if isIO t then frgn else runIO frgn)]
      where frgn = Lit $ LForImp (fromMaybe (unIdent (unQualIdent i)) ie) cty
            cty = CType t
            -- On the fun backend IO is CPS ((a -> r) -> r; see
            -- Primitives.primBind/primReturn), so RUNNING an action is applying
            -- it to the identity continuation -- the same shape genRomMem uses
            -- for the program entry. There is no runtime primitive to call.
            runIO e | rvfun flags = App e (Lit (LPrim "I"))
                    | otherwise   = App (Lit (LPrim "IO.performIO")) e
...
dsExpr aexpr =
  case aexpr of
    -- The --morph classifier marks a morphism head with a reserved EVar; lower
    -- it into the first-class Morph node (mirrors `./f`'s convF Sym "cata").
    EVar i | Just s <- lookupMorphMarker i -> Morph s
    EVar i | i == yMarker -> Lit (LPrim "Y")
    EVar i -> Var i
    ...
    -- fun backend: a String literal is a cons list of Char-code INTs (the C path
    -- keeps it packed as fromUTF8/LUStr; fun has no byte ops, so expand here).
    ELit _ (LStr cs) -> encList (map (Lit . LInt . ord) cs)
...
-- Pattern-match failure on the fun/nano backend: emit a bare error ATOM
-- (LPrim "errorN" -> GenRomMem.wErr, which halts the reducer). The old
-- Control.Exception.Internal.patternMatchFail dragged in Show/Typeable (full
-- Prelude); nano has no exception machinery, so a halting error atom is the
-- correct lowering and keeps NanoPrelude free of the typeclass hierarchy.
eMatchErr :: SLoc -> Exp
eMatchErr _ = Lit (LPrim "error7")
...
eLet i e b =
  if i == dummyIdent then
    ...
    case b of
      Var j | i == j -> e
      _ ->
        -- The single use substitution is essential for performance, but it
        -- is safe only when it cannot move WORK.  Substituting a thunk into
        -- an occurrence that sits under a lambda does move work: the binding
        -- is evaluated once and shared, the copy is evaluated on every entry
        -- to the lambda.  Measured on the reducer, a 33.5k value named once
        -- inside a ten-element map cost 322k substituted and 34.4k bound.  A
        -- value -- variable, literal, lambda -- is already in normal form and
        -- has no work to move, so it always goes in; a thunk goes in only
        -- where no lambda stands between.  The beta redex shares, so it is
        -- the safe fallback rather than a cost.
        case occInfo i b of
          (0, _) -> b             -- no occurences, no need to bind
          (1, underLam) | isValueExp e || not underLam -> substExp i e b
          _ -> App (Lam i b) e    -- just use a beta redex

-- A normal form: nothing to evaluate, so copying it costs no work.
isValueExp :: Exp -> Bool
isValueExp (App _ _) = False
isValueExp _         = True

-- How many free occurrences of `i` are in the term, saturating at two, and
-- does any of them lie beneath a lambda.  Both answers come from ONE walk
-- carrying a pair: asking `freeVars` and then a separate lambda test walks the
-- same term twice and builds a variable list in between only to count it.  A
-- binder of the same name shadows the occurrence, so that subtree holds none.
occInfo :: Ident -> Exp -> (Int, Bool)
occInfo i = go False
  where
    go inLam (Var j)   = if j == i then (1, inLam) else (0, False)
    go inLam (App f a) =
      case go inLam f of
        (nf, lf) ->
          case go inLam a of
            (na, la) -> (min 2 (nf + na), lf || la)
    go _     (Lam j b) = if j == i then (0, False) else go True b
    go _     _         = (0, False)
