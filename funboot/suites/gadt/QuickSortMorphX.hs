-- QuickSortMorphX.hs — quicksort via Morphx generic hylomorphism
-- (merged-fmap form).

module QuickSortMorphX (bench) where
import Prelude()
...
fmapQSF :: (a -> b) -> QSF a -> QSF b
fmapQSF _ QEmpty         = QEmpty
fmapQSF f (QPivot p l r) = QPivot p (f l) (f r)
...
qsort :: [W] -> [W]
qsort = hylo' fmapQSF concatAlg partitionCoalg
