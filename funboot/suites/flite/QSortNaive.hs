-- QSortNaive.hs - quickSort from fun-bmarks src/sorting/sorting.fn:
--   quickSort ls = ls Nil (\x xs -> concat (quickSort lo) (Cons x (quickSort hi))
--                                    (filter (\a -> leq a x) xs)
--                                    (filter (\a -> > a x) xs))
-- xs is traversed TWICE, once per filter.

module QSortNaive where
...
qsort :: [Int] -> [Int]
qsort []       = []
qsort (x : xs) = qsort (filter (\a -> a <= x) xs)
                 ++ (x : qsort (filter (\a -> a > x) xs))
