-- MultiplierSplit.hs - one pass to split, not three.
...
-- length + takeL + dropL is THREE walks of the list at every level of the gate
-- tree, to learn one thing: where the middle is.  One walk finds it and returns
-- both halves -- the same fix that made mergesort 1.62x (its split did length,
-- take and drop per level too).  A second cursor moving at double speed reaches
-- the end as the first reaches the middle.
splitHalf :: [B] -> ([B], [B])
splitHalf xs = go xs xs
  where
    go (y : ys) (_ : _ : zs) = case go ys zs of
                                 (l, r) -> (y : l, r)
    go ys       _            = ([], ys)
...
wideGate f [x] = x
wideGate f xs =
  case splitHalf xs of
    (l, r) -> f (wideGate f l) (wideGate f r)
