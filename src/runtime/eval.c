...
#if WANT_TIME
#include <time.h>
#include "fun_recipe.h"
#include <fcntl.h>
#include <unistd.h>

/* state behind the CPS IO atoms: the latched descriptor, the path being
 * accumulated a character at a time, and the argv cursor */
static int fun_io_fd = 1;
static char fun_io_path[1024];
static size_t fun_io_pathn;
static int fun_io_argsel = -1;
static int fun_io_argc;
static char **fun_io_argv;
static size_t fun_io_argpos;
#endif
...
#define VERSION "v1.0(sc)\n"   /* structured combinators: %<packed rvfun combi word> */
...
enum node_tag { T_FREE, T_IND, T_AP, T_INT, T_DBL, T_PTR, T_FUNPTR, T_FORPTR, T_BADDYN, T_ARR, T_BSTR,
                T_SC,
                T_IO_SETFD, T_IO_PUTBF, T_IO_PUTB, T_IO_GETB, T_IO_GETBF,
                T_IO_PATHC, T_IO_OPENF, T_IO_CLOSEF,
                T_IO_ARGC, T_IO_ARGSEL, T_IO_ARGRD,
                T_S, T_K, T_I, T_B, T_C,
...
static const char* tag_names[] = {
  ...
  "SC", "error7",
  "io.setfd", "io.putbf", "io.putb", "io.getb", "io.getbf",
  "io.pathc", "io.open", "io.close",
  "io.argc", "io.argsel", "io.argrd",
  "S", "K", "I", "B", "C",
...
/* primops */
  ...
  { "fpfin", T_FPFIN },
  //  { "fpstr", T_FPSTR },
  { "error7", T_MATCHERR },   /* the desugarer's pattern-match failure */
  { "seq", T_SEQ },
  ...
  { "raise", T_RAISE },
  { "catch", T_CATCH },
/* The CPS IO atoms.  This library's IO is continuation-passing -- an atom
 * takes its arguments AND the continuation directly, performs the effect and
 * continues -- rather than a distinct IO type the runtime interprets.  That is
 * expressible here: do the effect, then GOIND to the continuation. */
  { "io.setfd", T_IO_SETFD },
  { "io.putbf", T_IO_PUTBF },
  { "io.putb", T_IO_PUTB },
  { "io.getb", T_IO_GETB },
  ...
  { "io.pathc", T_IO_PATHC },
  { "io.open", T_IO_OPENF },
  { "io.close", T_IO_CLOSEF },
  { "io.argc", T_IO_ARGC },
  { "io.argsel", T_IO_ARGSEL },
  { "io.argrd", T_IO_ARGRD },
  { "A.alloc", T_ARR_ALLOC },
  { "A.copy", T_ARR_COPY },
  { "A.size", T_ARR_SIZE },
...
parse(BFILE *f)
{
  ...
  for(;;) {
    ...
    case '#':
      ...
      break;
    case '%':
      /* A structured combinator: the rvfun combi word, packed exactly as the
       * hardware encodes it -- arity, pattern number, and the argument
       * selectors.  One node type replaces the whole discrete S/K/I/B/C...
       * set; which application tree gets built is data, from fun_recipe[]. */
      i = parse_int(f);
      r = alloc_node(T_SC);
      SETVALUE(r, i);
      PUSH(r);
...
#define CMPP(op)       do { OPPTR2(r = xp op yp); GOIND(r ? combTrue : combFalse); } while(0)
 ...
 top:
   ...
   */
  case T_IO_SETFD: case T_IO_PUTBF: case T_IO_PUTB: case T_IO_PATHC:
  case T_IO_CLOSEF: case T_IO_ARGSEL: {
    /* Int -> a -> a: take the argument, do the effect, continue. */
    value_t v;
    NODEPTR k;
    tag_t t = tag;
    CHECK(2);
    v = evalint(ARG(TOP(0)));       /* before POP: this may collect */
    k = ARG(TOP(1));
    POP(2);
    n = TOP(-1);
    switch (t) {
    case T_IO_SETFD:  fun_io_fd = (int)v; if (getenv("IOTRACE")) fprintf(stderr, "[setfd %d]", (int)v); break;
    case T_IO_PUTBF:  { unsigned char b = (unsigned char)v;
                        if (getenv("IOTRACE")) fprintf(stderr, "[putbf %d]", (int)v);
                        ssize_t w = write(fun_io_fd, &b, 1); (void)w; } break;
    case T_IO_PUTB:   putchar((int)v); break;
    case T_IO_PATHC:  if (fun_io_pathn + 1 < sizeof(fun_io_path))
                        fun_io_path[fun_io_pathn++] = (char)v;
                      fun_io_path[fun_io_pathn] = 0; break;
    case T_IO_CLOSEF: close((int)v); break;
    case T_IO_ARGSEL: fun_io_argsel = (int)v; fun_io_argpos = 0; break;
    default: break;
    }
    GOIND(k);
  }

  case T_IO_GETB: case T_IO_GETBF: case T_IO_ARGC: case T_IO_ARGRD: {
    /* (Int -> a) -> a: do the effect, hand the result to the continuation. */
    value_t v = 0;
    NODEPTR k;
    tag_t t = tag;
    CHECK(1);
    ...
    n = TOP(-1);
    switch (t) {
    case T_IO_GETB:  { int c = getchar(); v = c; } break;
    case T_IO_GETBF: { unsigned char b; ssize_t r = read(fun_io_fd, &b, 1);
                       v = r == 1 ? b : -1; } break;
    case T_IO_ARGC:  v = fun_io_argc; break;
    case T_IO_ARGRD: { const char *a = (fun_io_argsel >= 0 &&
                                        fun_io_argsel < fun_io_argc)
                                       ? fun_io_argv[fun_io_argsel] : 0;
                       v = (a && a[fun_io_argpos]) ? (unsigned char)a[fun_io_argpos++] : -1;
                     } break;
    default: break;
    }
    GCCHECK(1);
  ...
  case T_IO_OPENF: {
    /* Int -> (Int -> a) -> a: open the accumulated path, hand back the fd. */
    value_t mode;
    NODEPTR k;
    int fd;
    CHECK(2);
    mode = evalint(ARG(TOP(0)));
    k = ARG(TOP(1));
    POP(2);
    n = TOP(-1);
    fd = open(fun_io_path, mode ? (O_WRONLY | O_CREAT | O_TRUNC) : O_RDONLY, 0666);
    fun_io_pathn = 0; fun_io_path[0] = 0;
    GCCHECK(1);
    GOAP(k, mkInt(fd));
  }
  ...
  case T_SC: {
    /*
     * The structured combinator.  Where the discrete set had one case per
     * combinator (S, K, I, B, C, S', B', C', ...), there is one case here and
     * the shape it builds comes from the recipe -- the same table the rvfun
     * hardware decodes, generated from Core/Fun.hs.
     *
     * A recipe writes a flat run of words that the machine reads as
     * application blocks, "[link a][link b][elink f]" meaning f applied to its
     * arguments; the words push in order, so the LAST pushed is f's first
     * argument.  Here the heap is binary cells, so each block becomes a
     * left-nested chain of AP nodes.  Blocks refer to each other by word
     * offset and those references go both ways, so the chains are allocated
     * first and filled afterwards.  `copy` names the base the redex root
     * becomes, which is exactly this evaluator's GOIND.
     */
    uint32_t cw = (uint32_t)GETVALUE(n);
    int arity = (cw >> 11) & 7;
    int type  = (cw >> 4) & 0x7f;
    const FunRecipe *r;
    NODEPTR bases[16];
    NODEPTR grp[10];
    int gstart[10], ngrp = 0;
    int k, j, cells = 0;

    if (type >= FUN_RECIPE_N)
      ERR("SC: no recipe");
    r = &fun_recipe[type];
    CHECK(arity);

    /* one AP cell per argument in each block */
    for (k = 0, j = 0; k < r->hw_n; k++) {
      if (r->hw_t[k] & 1) { cells += k - j; j = k + 1; }
    }
    GCCHECK(cells);

    /* bases 0..5: the arguments this word selects, counted from the top */
    for (k = 0; k < 6; k++) {
      int i = (cw >> (14 + 3 * k)) & 7;
      bases[k] = i < arity ? ARG(TOP(i)) : NIL;
    }

    /* allocate a chain per block, then fill: references run both ways */
    for (k = 0, j = 0; k < r->hw_n; k++) {
      if (r->hw_t[k] & 1) {
        int nargs = k - j, a;
        NODEPTR t = NIL;
    ...
    for (k = 0; k < 9; k++) {
      bases[6 + k] = NIL;
      for (j = 0; j < ngrp; j++)
        if (gstart[j] == k) bases[6 + k] = grp[j];
    }
    bases[15] = NIL;

    for (j = 0, k = 0; j < ngrp; j++) {
      int st = gstart[j], en = st, a, nargs;
      while (!(r->hw_t[en] & 1)) en++;
      nargs = en - st;
      if (!nargs) {
        bases[6 + st] = bases[r->hw_b[en]];       /* a block with no arguments */
      } else {
        NODEPTR t = grp[j];
        /* outermost holds word st, innermost applies the head to word en-1 */
        for (a = 0; a < nargs; a++) {
          NODEPTR inner = (a + 1 < nargs) ? FUN(t) : NIL;
          ARG(t) = bases[r->hw_b[st + a]];
          if (a + 1 == nargs)
            FUN(t) = bases[r->hw_b[en]];
          t = inner;
        }
      }
      (void)k;
    }
    /* a no-argument block resolved to another base: settle those references */
    for (k = 0; k < 9; k++) {
      for (j = 0; j < ngrp; j++)
        if (gstart[j] == k && grp[j] == NIL) {
          int en = gstart[j];
          while (!(r->hw_t[en] & 1)) en++;
          bases[6 + k] = bases[r->hw_b[en]];
        }
    }
...
MAIN
{
  ...
  gargc = av - gargv;
  fun_io_argc = gargc;
  fun_io_argv = gargv;   /* the CPS argv atoms read these */
  ...
  topnode = &TOP(0);
  /*
   * This library's IO is continuation-passing: main is a function that takes
   * the rest of the program, not an IO action for execio() to interpret.  So
   * apply it to a continuation and evaluate -- the effects happen as the io.*
   * atoms are reduced, and the continuation ends it by returning a WHNF.
   */
  GCCHECK(4);
  if (getenv("IOTRACE"))
    fprintf(stderr, "[prog tag %d]", (int)GETTAG(TOP(0)));
  /* CPS main takes the continuation; the continuation itself is applied to the
   * result, so give it one that swallows that argument too. */
  TOP(0) = new_ap(TOP(0), new_ap(alloc_node(T_K), alloc_node(T_I)));
  {
    NODEPTR res = evali(TOP(0));
    if (getenv("IOTRACE")) {
      NODEPTR h = res; int d = 0;
      while (GETTAG(h) == T_AP && d < 20) { h = FUN(h); d++; }
      fprintf(stderr, "[res tag %d, head tag %d after %d args]\n",
              (int)GETTAG(res), (int)GETTAG(h), d);
      if (GETTAG(h) == T_SC)
        fprintf(stderr, "[stuck SC word %u arity %u]\n",
                (unsigned)GETVALUE(h), (unsigned)((GETVALUE(h) >> 11) & 7));
    }
  }
  prog = TOP(0);
  POP(1);
  /* CPS main returns whatever its continuation produced; there is no
   * IO_RETURN wrapper to check for. */
