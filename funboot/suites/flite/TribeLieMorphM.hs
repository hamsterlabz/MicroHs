-- TribeLieMorphM.hs - TribeLie in the exp3_8m idiom.
module TribeLieMorphM where
...
-- `sum $ map (\x -> if p x then 1 else 0) xs` builds a list of 0s and 1s the
-- same length as xs, then folds it away.  One catamorphism carrying the count
-- builds nothing -- the Mss pattern.
count p = fix (\ rec xs ->
                 case xs of
                   []       -> 0::Int
                   (y : ys) -> (if p y then (1::Int) else 0) + rec ys)
