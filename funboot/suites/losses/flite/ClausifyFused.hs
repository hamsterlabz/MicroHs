-- ClausifyFused.hs - Clausify with its two tree passes fused.
--
-- clausify is a six-stage pipeline, every stage building an intermediate that
-- the next stage immediately walks.  The two tree stages, disin . negin, are
-- catamorphisms over the same Expr, so they fuse exactly.
module ClausifyFused where
...
-- disin . negin are two catamorphisms over the SAME tree, so the intermediate
-- negin-tree exists only to be walked by disin.  Fused into one pass, derived
-- case by case:
--   negin (Neg (Con p q)) = Dis (negin (Neg p)) (negin (Neg q))  then
--   disin (Dis a b)       = din (disin a) (disin b)
-- so  dn (Neg (Con p q)) = din (dn (Neg p)) (dn (Neg q)),  and so on.
dn :: Expr Int -> Expr Int
dn (Sym s)          = Sym s
dn (Neg (Sym s))    = Neg (Sym s)
dn (Neg (Con p q))  = din (dn (Neg p)) (dn (Neg q))
dn (Neg (Dis p q))  = Con (dn (Neg p)) (dn (Neg q))
dn (Neg (Neg p))    = dn p
dn (Dis p q)        = din (dn p) (dn q)
dn (Con p q)        = Con (dn p) (dn q)
