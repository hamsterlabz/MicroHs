...
-- Read up to n bytes. Short (possibly empty) at end of file - which is what
-- the lazy flavour reads as "the file ended". Built on pack rather than on
-- the array primitives so it stays inside this module's imports; the list is
-- one chunk long and dies with it.
hGet :: Handle -> Int -> IO ByteString
hGet h n = fmap pack (go n)
  where go k = if k <= (0::Int) then P.return [] else do
                 e <- P.hIsEOF h
                 if e then P.return [] else do
                   c <- P.hGetChar h
                   ws <- go (k - 1)
                   P.return (P.fromIntegral (P.fromEnum c) : ws)
